...
app.options('*', cors()); // Trả lời preflight OPTIONS cho mọi route
app.use(express.json());

// ── DEBUG: log admin requests để kiểm tra secret nhận được ──
app.use('/api/admin', (req, res, next) => {
  const secret = req.query?.secret || req.headers['x-admin-secret'] || req.body?.secret;
  console.log(`[Admin] ${req.method} ${req.path} | secret_match=${secret === process.env.ADMIN_SECRET} | source=${req.query.secret ? 'query' : req.headers['x-admin-secret'] ? 'header' : 'body'}`);
  next();
});
